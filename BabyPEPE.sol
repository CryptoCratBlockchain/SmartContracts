...
contract BabyPEPE is ERC20, Ownable {
    ...
    function updateBuyFees(uint256 _marketingFeeOnBuy, uint256 _rewardFeeOnBuy) external onlyOwner {
        require(_marketingFeeOnBuy + _rewardFeeOnBuy <= 10, "Fees must be less than 10%");       
    ...
    function updateSellFees(uint256 _marketingFeeOnSell, uint256 _rewardFeeOnSell) external onlyOwner {       
        require(_marketingFeeOnSell + _rewardFeeOnSell <= 10, "Fees must be less than 10%");       
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && to == uniswapV2Pair) {
            ...
            uint256 ETHShare = marketingShare + rewardShare;
            ...
            if (contractTokenBalance > 0 && ETHShare > 0) {
                ...
                if (rewardShare > 0) {
                    uint256 rewardETH = (newBalance * rewardShare) / ETHShare;
                    swapAndSendDividends(rewardETH);  
                    emit SendDividends(rewardETH);                  
                }               
            }
